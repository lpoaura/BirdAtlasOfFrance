...
$$
    BEGIN
        ...
        INSERT INTO atlas.t_historic_atlases_info (id_territory, atlas_period, date_start, date_end, season_period,
                                                   description)
        SELECT DISTINCT territory.id_area,
                        EXTRACT(YEAR FROM irc.date_inf::DATE) || '-' ||
                        EXTRACT(YEAR FROM irc.date_sup::DATE) AS atlas_period,
                        irc.date_inf::DATE,
                        irc.date_sup::DATE,
        ...
        INSERT INTO atlas.t_historic_atlases_info (id_territory, atlas_period, date_start, date_end, season_period,
                                                   description)
        SELECT id_area               AS id_territory,
        ...
        FROM ref_geo.l_areas AS territory,
        ...
        BEGIN
            ...
            SELECT t_historic_atlases_info.id AS id_historic_atlas_info,
                   grid.id_area,
                   taxref.cd_ref,
                   CASE
                       WHEN ir.indice_syn_label::TEXT = 'Possible'::TEXT THEN
            ...
            FROM src_historic_atlas.i037_releves rel
                     JOIN atlas.t_historic_atlases_info ON (atlas_period =
                                                            EXTRACT(YEAR FROM rel.date_inf::DATE) || '-' ||
            ...
            INSERT INTO atlas.t_historic_atlases_data (id_historic_atlas_info, id_area, cd_nom, status)
            SELECT t_historic_atlases_info.id,
                   id_area,
                   cd_nom,
                   COALESCE(
                           CASE
                               WHEN statut_nidif ILIKE 'nicheur possible'::TEXT THEN
                                   'Nicheur possible'::TEXT
            ...
            FROM tmp.aofm_2009_2012
                     ...
                     JOIN atlas.t_historic_atlases_info ON ((
                                                                CASE
                                                                    WHEN aofm_2009_2012.statut_nidif IS NOT NULL THEN
                                                                        'breeding'
                                                                    ELSE
                                                                        'wintering'
                                                                    END)::phenology_period =
                                                            t_historic_atlases_info.season_period
