...
$$
    BEGIN
        ...
        VALUES (4, 'ODF Noms', 'Informations relatives à Oiseaux de France (commun)', 1, 3)
        ...
        */
        INSERT INTO taxonomie.bib_attributs ( id_attribut
                                            , nom_attribut
                                            , label_attribut
        ...
        VALUES ( 202
             ...
             , ( 205
               ...
               , 5
               , 2)
             , (206, 'feeding', 'Alimentation', '{}', FALSE, 'Alimentation', 'text', 'textarea', NULL, NULL, 5, 3)
             , (207, 'habitat', 'Habitat', '{}', FALSE, 'Habitat', 'text', 'textarea', NULL, NULL, 5, 3)
             , (208, 'breeding', 'Reproduction', '{}', FALSE, 'Reproduction', 'text', 'textarea', NULL, NULL, 5, 4)
             , (209, 'common_synonyms', 'Synonymes', '{}', FALSE, 'Synonymes', 'text', 'text', NULL, NULL, 4, 4)
        ON CONFLICT DO NOTHING;

        SELECT * FROM taxonomie.bib_attributs;
        DELETE FROM taxonomie.cor_taxon_attribut WHERE id_attribut < 200;

        /* INFO: Ajout des types de médias spécifiques */
        ALTER TABLE taxonomie.bib_types_media
            ADD UNIQUE (nom_type_media);

        INSERT INTO taxonomie.bib_types_media(id_type, nom_type_media, desc_type_media)
        VALUES ((SELECT max(id_type) + 1 FROM taxonomie.bib_types_media), 'xeno-canto', 'Lien https://xeno-canto.org')
        ON CONFLICT (nom_type_media) DO NOTHING;

        INSERT INTO taxonomie.bib_types_media(id_type, nom_type_media, desc_type_media)
        VALUES ( (SELECT max(id_type) + 1 FROM taxonomie.bib_types_media)
               , 'euro-bird-portal'
...
--                           , taxref.nom_vern
                         ...
                         FROM src_vn_json.species_json
                                  ...
                                  JOIN taxonomie.taxref ON cor_c_vn_taxref.taxref_id = taxref.cd_nom
                         WHERE site = (SELECT DISTINCT site
                                       FROM src_vn_json.species_json
                                       LIMIT 1)
           ...
           , attributs AS (SELECT (SELECT id_attribut
                           ...
                           SELECT (SELECT id_attribut
                                   FROM taxonomie.bib_attributs
                                   WHERE bib_attributs.nom_attribut LIKE 'common_synonyms') AS id_attribut
                                , synonyms                                                  AS valeur_attribut
                                , species.cd_nom
                           FROM species)
        INSERT
        INTO taxonomie.cor_taxon_attribut(id_attribut, valeur_attribut, cd_ref)
        SELECT DISTINCT id_attribut, (array_agg(valeur_attribut))[1], cd_nom
        FROM attributs
        GROUP BY cd_nom, id_attribut
        ORDER BY cd_nom, id_attribut
        ON CONFLICT (id_attribut, cd_ref)
            DO UPDATE
            SET valeur_attribut = excluded.valeur_attribut
        WHERE (excluded.id_attribut, excluded.cd_ref) = (cor_taxon_attribut.id_attribut, cor_taxon_attribut.cd_ref);
        ...
        GROUP BY taxref.cd_nom
               , taxref.cd_ref
        ON CONFLICT (cd_nom) DO NOTHING;

        /* INFO : ajout des liens xeno-canto aux médias des espèces */
        INSERT INTO taxonomie.t_medias (cd_ref, titre, url, chemin, auteur, desc_media, date_media, id_type, source,
                                        licence)
        SELECT t_taxa.cd_nom
        ...
        WHERE nom_type_media = 'xeno-canto'
          AND t_taxa.enabled
...
--                     JOIN taxonomie.taxref ON t_taxa.cd_nom = taxref.cd_nom
           ...
           , taxonomie.bib_types_media
        WHERE nom_type_media = 'euro-bird-portal'
          AND t_taxa.enabled
          AND t_taxa.available;
