...
AS $function$          
    ...
    BEGIN
        ...
        COPY odf_imports.natureocentre_data(entity_source_pk_value, uuid, cd_nom, altitude_min, date_min, bird_breed_code,
                                     "Migration active", cd_nomenclature_observation_status, geom, cd_sig, commentaire,
		...
		INSERT INTO gn_synthese.synthese (unique_id_sinp, unique_id_sinp_grp, id_source, id_module, entity_source_pk_value,
		                                  id_dataset, grp_method, id_nomenclature_diffusion_level, id_nomenclature_sensitivity,
		                                  id_nomenclature_observation_status, id_nomenclature_valid_status,
		                                  reference_biblio, count_min, count_max, cd_nom, cd_hab, nom_cite, sample_number_proof,
		                                  ...
		                                  id_area_attachment, date_min, date_max, validator, validation_comment, observers,
		                                  determiner, id_digitiser, comment_context, comment_description, additional_data,
		...
		FROM odf_imports.natureocentre_data noc
		     ...
		     gn_meta.t_datasets
		WHERE name_source = 'Nature O Centre' AND dataset_shortname LIKE 'natureocentre' AND date_maj::date >= the_last_update::date
		...
		INSERT INTO src_lpodatas.t_c_synthese_extended(id_synthese,
		                                               ...
		                                               date_year, is_valid)
		WITH t1 AS (SELECT id_synthese, string_to_array(entity_source_pk_value, '|') AS entity_source_pk_values, date_min
		            FROM gn_synthese.synthese
		            WHERE id_source = 34)
		SELECT id_synthese,
		       max(bird_breed_code),
		       ref_nomenclatures.get_nomenclature_label_by_cdnom_mnemonique('VN_ATLAS_CODE',max(bird_breed_code)::TEXT),
	    ...
	    SELECT max(date_sup) INTO the_last_delete 
        ...
        FROM '/home/gestionnaire/sql_scripts/do_not_delete/natureOcentre/supodf.csv' HEADER  CSV DELIMITER E'\t';
        /*Suppression des données dans la synthese et extended_synthese*/
        DELETE
FROM
	gn_synthese.synthese
WHERE
	entity_source_pk_value IN (
	SELECT
		entity_source_pk_value::varchar
	FROM
		odf_imports.natureocentre_data_delete
