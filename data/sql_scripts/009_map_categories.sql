...
$$
    BEGIN
        ...
        WITH q1 AS (SELECT 'all_period'                                                                             AS period
                         , gtm.id_area_territory                                                                    AS group_id_area
                         , gtm.id_area_grid                                                                         AS id_area
                         , akl.allperiod_count_taxa_new                                                             AS count_taxa
                         , NTILE(5)
                           OVER (PARTITION BY gtm.id_area_territory ORDER BY akl.allperiod_count_taxa_new)          AS ntile
                    FROM atlas.mv_grid_territories_matching gtm
                             LEFT JOIN atlas.mv_area_knowledge_level akl ON gtm.id_area_grid = akl.id_area
                    WHERE akl.allperiod_count_taxa_new > 0
                    UNION
                    SELECT 'wintering'                                                                              AS period
                         , gtm.id_area_territory                                                                    AS group_id_area
                         , gtm.id_area_grid                                                                         AS id_area
                         , akl.wintering_count_taxa_new                                                             AS count_taxa
                         , NTILE(5)
                           OVER (PARTITION BY gtm.id_area_territory ORDER BY akl.wintering_count_taxa_new)          AS ntile
                    FROM atlas.mv_grid_territories_matching gtm
                             LEFT JOIN atlas.mv_area_knowledge_level akl ON gtm.id_area_grid = akl.id_area
                    WHERE akl.wintering_count_taxa_new > 0

                    UNION
                    SELECT 'breeding'                                                                              AS period
                         , gtm.id_area_territory                                                                   AS group_id_area
                         , gtm.id_area_grid                                                                        AS id_area
                         , akl.breeding_count_taxa_new                                                             AS count_taxa
                         , NTILE(5)
                           OVER (PARTITION BY gtm.id_area_territory ORDER BY akl.breeding_count_taxa_new)          AS ntile
                    FROM atlas.mv_grid_territories_matching gtm
                             LEFT JOIN atlas.mv_area_knowledge_level akl ON gtm.id_area_grid = akl.id_area
                    WHERE akl.breeding_count_taxa_new > 0)
        ...
        GROUP BY period, group_id_area, ntile
        ORDER BY period, group_id_area, ntile
        WITH NO DATA;
